...
const CONTENT_DIR = path.join(__dirname, 'data', 'being-sevak-content');
const NAV_FILE = path.join(__dirname, 'data', 'being-sevak-nav.json');
const FOOTER_FILE = path.join(__dirname, 'data', 'being-sevak-footer.json');

// Generic template pages that are not part of the live Being Sevak route map.
// They get removed (if present) so the CMS mirrors the actual website only.
const GENERIC_SLUGS = ['projects', 'gallery', 'documents', 'faq', 'privacy-policy'];
  ...
  for (const page of pages) {
    for (const section of page.sections) {
      ...
      const data = {
        ...
        description: `Section template for "${section.type}" on the Being Sevak website.`,
...
async function main(): Promise<void> {
  ...
  console.log(`Website content seeded: ${seeded.size} pages, ${pages.reduce((n, p) => n + p.sections.length, 0)} sections, ${files.length} templates.`);
}
