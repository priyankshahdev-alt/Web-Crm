...
export function Pagination({ page, totalItems, pageSize, onChange, noun = 'items' }: PaginationProps) {
    ...
    return (
      <div className="flex items-center justify-between border-t border-line px-5 py-4">
        <p className="text-sm text-muted">
          Showing {totalItems} {totalItems === 1 ? noun.replace(/s$/, '') : noun}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-line px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-muted">
        ...
        <span className="font-semibold text-ink">{totalItems}</span> {noun}
      </p>
      <div className="flex items-center gap-1">
        <button
          ...
          disabled={safePage <= 1}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-line text-muted transition-colors duration-150 hover:bg-soft hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-brand/40 disabled:cursor-not-allowed disabled:opacity-40"
        >
        ...
        {pageItems.map((item) => {
          if (item === 'left-ellipsis' || item === 'right-ellipsis') {
            return (
              <span key={item} aria-hidden="true" className="flex h-9 w-9 items-center justify-center text-sm text-faint">
                …
              </span>
            )
          }
